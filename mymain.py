...
def crossover(par_a, par_b):
    par_a_cfg = load_yaml(f'{args.new}/{par_a}.yaml')
...
def run_NSGA2(metrics, inshape, p_size = 2, q_size = 2, generations=5, mutation_rate = 0.1):
    ...
    global cfg, ss
    cfg = load_yaml(args.load_cfg)
    ss = load_yaml(args.load_ss)
    ...
    for g in range(generations):
        ...
        for i in range(int(q_size/2)):
            ...
            # not handled ... !
            ''' # todo: this part can go to infinite loop
            cnt=0
            while par_b["gene"][:]==par_a["gene"][:] or par_a["gene"][len(par_a["gene"])-3][0]!=par_b["gene"][len(par_b["gene"])-3][0] or cnt<100:
                ...
                if cnt==99:
                    break
            # the above while loop tries to find a partner for the parent but will not get out of the loop if cnt >= 99 
            if cnt>98:
                mut=[[],[]]
...
def train(model:YOLO, data, args):
    """
    data: for yolo it's just a .yaml file name/path
...
if __name__ == "__main__":
    ...
    # add help arg for each of them, if you feeling lucky :)
    parser.add_argument('--load_cfg', default='recipe/yolov8n.yaml')
    parser.add_argument('--load_ss', default='recipe/ss.yaml')
    ...
    parser.add_argument('--save_dir', default='results/data')
    parser.add_argument('--cache_dir', default='results/cache')
